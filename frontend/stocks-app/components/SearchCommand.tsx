...
export default function SearchCommand({
  ...
  // Fetch popular stocks when dialog opens (only if not cached)
  useEffect(() => {
    if (open && stocks.length === 0 && popularStocksCache.length === 0) {
      ...
      // Use cached popular stocks
      setStocks(popularStocksCache)
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [open])

  // Search stocks when user types (with debounce)
  useEffect(() => {
    // Only run search logic when dialog is open
    if (!open) return
    ...
    const delayDebounce = setTimeout(() => {
      if (searchTerm.trim()) {
        ...
        // Use cache when search cleared instead of fetching again
